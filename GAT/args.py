...
def parser_loader():
    ...
    parser.add_argument("--spar_wei", default=True, action='store_true')
    parser.add_argument("--spar_adj", default=True, action='store_true')
    ...
    parser.add_argument('--remain', default=ed('REMAIN',1.0), type=float,
                        help='percentage of remained parameters allowed. if None, pruning will not be used. must be on the interval (0, 1]')
    parser.add_argument('--delta', default=ed('DELTA', 10), type=int,
                        help='delta param for pruning')
    parser.add_argument('--accumulation-n', default=ed('ACCUMULATION_N', 9), type=int,
    ...
    parser.add_argument('--warmup_steps', default=ed('WARMUP_STEPS', 3), type=int, help='if 0, completely one shot')
    
    
    args = vars(parser.parse_args())
    seed_dict = {'cora': 1999, 'citeseer': 17889, 'pubmed': 3333}
